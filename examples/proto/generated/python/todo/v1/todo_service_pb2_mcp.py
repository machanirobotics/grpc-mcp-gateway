...
def register_todo_service_mcp_handler(server: Server, impl: TodoServiceMCPServer) -> None:
    ...
    @server.call_tool()
    async def handle_call_tool(name: str, arguments: dict[str, Any]) -> list[types.TextContent | types.ResourceContent]:
        if name == "todo_service-create_todo_v1":
            ...
            resp_dict = MessageToDict(resp, preserving_proto_field_name=True, always_print_fields_with_no_presence=True)
            result = [types.TextContent(type="text", text=json.dumps(resp_dict))]
            return result
