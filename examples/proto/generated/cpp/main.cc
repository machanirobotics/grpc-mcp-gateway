...
int main() {
    ...
    todo::v1::start_grpc_server(grpc_addr);
    std::this_thread::sleep_for(std::chrono::milliseconds(200));

    if (transport == "stdio") {
        std::cerr << "MCP attaching (transport=stdio)" << std::endl;
        todo::v1::start_todo_service_mcp_stdio();
    } else {
        auto host = env_or("MCP_HOST", "0.0.0.0");
        auto port_s = env_or("MCP_PORT", "8082");
        auto base = env_or("MCP_BASE_PATH", "/todo/v1/todoservice/mcp");
        ...
        todo::v1::start_todo_service_mcp_http(host, port, base);
    }
    return 0;
}
